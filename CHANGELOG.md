...
- Hardened the Linear execution contract so direct GraphQL remains primary when
  credentials are available, while MCP is now a required fallback when API
  execution is unavailable, blocked, unimplemented, rejected, or cannot prove
  read-back.
- Added expanded-mode outer-harness dogfood docs for Symphony, harness
  guides/sensors, recursive Ralph/autoresearch loops, API/MCP fallback
  decisions, QA evidence, and dependency mapping.
- Added recursive loop and sensor-timing guidance to validation, expanded-mode,
  source-checkpoint, and ledger-template surfaces.
- Strengthened evaluator coverage for API/MCP fallback wording and dogfood
  source-checkpoint attachment read-back.
